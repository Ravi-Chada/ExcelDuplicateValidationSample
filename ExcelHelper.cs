public class Script : ScriptBase
{
    public override async Task<HttpResponseMessage> ExecuteAsync()
    {
        ...
        if(this.Context.OperationId == "ChunkArray")
        {
         ...
         if(this.Context.OperationId == "UpdateArray")
        {
            return await this.HandleUpdateArray().ConfigureAwait(false);
        }
    ...
    private async Task<HttpResponseMessage> HandleUpdateArray()
    {
        //indexlist should be comma-separated integers
        var indexList = this.Context.Request.Headers.TryGetValues("IndexList", out var indexListValue) ? indexListValue.FirstOrDefault(): "";
        var elementIdx = this.Context.Request.Headers.TryGetValues("ElementIndex", out var elementIndex) ? elementIndex.FirstOrDefault(): "0";
        var updateValue = this.Context.Request.Headers.TryGetValues("UpdateValue", out var propValue) ? propValue.FirstOrDefault(): "0";
    
        string[] indexValues = indexList.Split(',');
        int[] indexArray = Array.ConvertAll(indexValues, int.Parse);

        int colIdx = int.Parse(elementIdx);
        ...
        {
            if(i >= 0 && i < dataArray.Count) dataArray[i][colIdx] = updateValue;            
        }

        HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.OK);
        response.Content = CreateJsonContent(dataArray.ToString());
        return response;
    }
}
